...
Replace `<your-sonatype-guide-api-token-here>` with your sonatype token which you can create from your https://guide.sonatype.com/ account.

=== Unused Deps
Pomegranate should only be including deps that are needed.
Check for unused deps via:
...
Some deps are not directly called from code but are still needed.
Adjust explanations for these deps in `bb.edn` under `unused-deps` task.
