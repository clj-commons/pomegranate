...
NOTE: *Legacy Logging Dependency* Pomegranate includes a legacy dependency on `commons-logging/commons-logging` to avoid breaking existing applications that rely on it transitively.
Modern applications should exclude this dependency (e.g., with `:exclusions [commons-logging/commons-logging]`) and instead add a bridge like `org.slf4j/jcl-over-slf4j` to delegate logging to SLF4J.
...
For usage in a modern app, you should exlude `commons-logging` (see note above):
[source,clojure,subs="attributes+"]
----
[clj-commons/pomegranate "{lib-version}" :exclusions [commons-logging/commons-logging]]
----
...
[source,clojure,subs="attributes+"]
----
clj-commons/pomegranate {:mvn/version "{lib-version}" :exclusions [commons-logging/commons-loggin]}
----
...
For usage in a modern app, you should exclude `commons-logging`, see `deps.edn` example above.
